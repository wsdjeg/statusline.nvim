# statusline.nvim - Neovim Plugin Assistant

I help with Lua plugin development for statusline.nvim, remember our conversations, and keep things simple.
...
- Module functions: `M.function_name` (camelCase for public API)
- Internal helpers: `local function name()`
- Use `vim.tbl_deep_extend` for config merging
- Highlight groups follow `SpaceVim_statusline_*` naming convention
- Always return `M` at the end of modules
...
statusline.nvim/
├── lua/statusline/
│   ├── init.lua          # Main module: setup, statusline rendering, mode handling
│   ├── config.lua         # Configuration management
│   └── util.lua           # Utility functions: separators, colors, build
├── test/
