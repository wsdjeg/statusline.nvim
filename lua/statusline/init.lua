...
local config = require('statusline.config').get()
...
local colors_template = default_color_palette()
...
function M.winnr(id)
  return util.circled_num(id, config.index_type)
end
...
local function current_tag()
  return '%{ v:lua.require("statusline")._current_tag() }'
end
...
local function active()
  ...
  local rsec = {}
  for _, section in ipairs(config.right_sections) do
    if registed_sections[section] then
      local rst = ''
      local ok, _ = pcall(function()
        if type(registed_sections[section]) == 'function' then
...
local special_statusline = {
  ...
  startify = function()
    ...
    local st = '%#SpaceVim_statusline_ia#'
      ...
      .. lsep
    if index(config.left_sections, 'vcs') ~= -1 and registed_sections.vcs then
