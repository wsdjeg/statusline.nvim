...
local modes = {
	...
	["fill-column-indicator"] = {
		icon = util.circled_letter("f"),
		icon_asc = "f",
		desc = "fill-column-indicator mode",
	},
	["syntax-checking"] = {
		icon = util.circled_letter("s"),
		icon_asc = "s",
	...
	["paste-mode"] = {
		icon = util.circled_letter("p"),
		icon_asc = "p",
		desc = "paste mode",
	},
	whitespace = {
		icon = util.circled_letter("w"),
		icon_asc = "w",
...
function M.winnr(id)
	return util.circled_num(id, vim.g.spacevim_windows_index_type)
end
...
local function fileformat()
	...
	return '%{ " " . g:_spacevim_statusline_fileformat . " ' .. irsep .. ' " . (&fenc!=""?&fenc:&enc) . " "}'
end
...
local function major_mode()
	local alias = get_alias(vim.o.filetype)
	if alias == "" then
		return ""
	else
		return " " .. alias .. " "
...
local function current_time()
	return " " .. util.current_time() .. " "
end

local function current_date()
	return " " .. util.current_date() .. " "
end
...
local function inactive()
	...
	for _, sec in ipairs(secs) do
		...
		base = base + len
		l = l .. '%{ get(w:, "winwidth", 150) < ' .. base .. ' ? "" : (" ' .. util.eval(sec) .. " " .. ilsep .. '")}'
	end
...
local special_statusline = {
	...
	vimchat = function()
		return "%#SpaceVim_statusline_ia#"
			...
			.. "%#SpaceVim_statusline_ia_SpaceVim_statusline_b#"
			.. lsep
			.. "%#SpaceVim_statusline_b# Chat %#SpaceVim_statusline_b_SpaceVim_statusline_c#"
			.. lsep
			.. "%#SpaceVim_statusline_c# "
			.. "%{chat#windows#status().protocol}"
			.. " %#SpaceVim_statusline_c_SpaceVim_statusline_b#"
			.. lsep
			.. "%#SpaceVim_statusline_b# "
			.. "%{chat#windows#status().channel}"
			.. " %#SpaceVim_statusline_b_SpaceVim_statusline_c#"
			.. lsep
			.. "%#SpaceVim_statusline_c# "
	...
	["denite-filter"] = function()
		return "%#SpaceVim_statusline_a_bold#" .. " Filter " .. "%#SpaceVim_statusline_a_SpaceVim_statusline_b#" .. lsep
	end,
...
function M.def_colors()
	...
	util.hi_separator("SpaceVim_statusline_a", "SpaceVim_statusline_b")
	util.hi_separator("SpaceVim_statusline_a_bold", "SpaceVim_statusline_b")
	util.hi_separator("SpaceVim_statusline_ia", "SpaceVim_statusline_b")
	util.hi_separator("SpaceVim_statusline_b", "SpaceVim_statusline_c")
	util.hi_separator("SpaceVim_statusline_b", "SpaceVim_statusline_z")
	util.hi_separator("SpaceVim_statusline_c", "SpaceVim_statusline_z")
end
...
function M.config()
	...
	then
		local conf = vim.json.decode(
			vim.fn.join(vim.fn.readfile(vim.fn.expand(vim.g.spacevim_data_dir .. "SpaceVim/major_mode.json"), ""), "")
		)
...
function M.mode(mode)
	...
	if vim.w.spacevim_statusline_mode ~= mode then
		...
		elseif mode == "v" or mode == "V" or mode == #"" or mode == "s" or mode == "S" or mode == #"" then
			vim.api.nvim_set_hl(0, "SpaceVim_statusline_a", {
				bold = true,
				fg = t[6][1],
				bg = t[6][2],
...
function M.mode_text(mode)
	...
	elseif mode == "t" then
		mode_text = "   TERM"
	elseif mode == "v" or mode == "V" or mode == "^V" or mode == "s" or mode == "S" or mode == "^S" then
		mode_text = " VISUAL"
	end
